    ...
    public class VideoRepository : BaseRepository, IVideoRepository
    {
        ...
        public Video GetById(int id)
        {
            using (var conn = Connection)
            {
                ...
                {
                    cmd.CommandText = @"
                           SELECT v.Title, v.Description, v.Url, v.DateCreated As VideoCreateDate, UserProfileId, 
                            up.Name, up.Email, up.DateCreated AS UserProfileDateCreated,
                            up.ImageUrl AS UserProfileImageUrl
                            FROM Video v Join UserProfile up
                    ...
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        ...
                        if (reader.Read())
                        {
                            video = new Video()
                            {
                                ...
                                Title = DbUtils.GetString(reader, "Title"),
                                Description = DbUtils.GetString(reader, "Description"),
                                DateCreated = DbUtils.GetDateTime(reader, "VideoCreateDate"),
                                Url = DbUtils.GetString(reader, "v.Url"),
                                UserProfileId = DbUtils.GetInt(reader, "UserProfileId"),
                                UserProfile = new UserProfile()
                                {
                                    Id = DbUtils.GetInt(reader, "UserProfileId"),
        ...
        public List<Video> SearchByDate(DateTime Since)
        {
            using (var conn = Connection)
            {
                conn.Open();
